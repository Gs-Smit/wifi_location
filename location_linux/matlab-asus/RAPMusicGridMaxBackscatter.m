...
maxCorr = [];
if ~useNoise
    if ~isempty(delayFromMusicPrev)  %delayFromMusicPrev不为空
        Ahat = [];
    ...
    PsA = Ps*PerpAhat_a;
    music_spec_num = sum((PerpAhat_a').*(PsA.'),2);            % now conjugate of each a1 should be dot producted with columns of PsA   (A.' 非共轭转置  A' 共轭转置)
